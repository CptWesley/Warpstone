...
namespace Warpstone
{
    ...
    public static class ExpressionParser
    {
        ...
        public static Operation<TExpression, TOperator> LeftToRight<TExpression, TOperator>(TOperator op, Func<TExpression, TExpression, TExpression> transformation)
        ...
        /// <returns>A left-to-right associative operation.</returns>
        public static Operation<TExpression, TOperator> LeftToRight<TExpression, TOperator>(Dictionary<TOperator, Func<TExpression, TExpression, TExpression>> transformations)
        ...
        public static Operation<TExpression, TOperator> RightToLeft<TExpression, TOperator>(TOperator op, Func<TExpression, TExpression, TExpression> transformation)
        ...
        /// <param name="transformations">The transformations.</param>
        /// <returns>A right-to-left associative operation.</returns>
        public static Operation<TExpression, TOperator> RightToLeft<TExpression, TOperator>(Dictionary<TOperator, Func<TExpression, TExpression, TExpression>> transformations)
        ...
        /// <param name="operatorParser">The operator parser.</param>
        /// <param name="operations">The operations.</param>
        /// <returns>A parser parsing a binary expression recursively.</returns>
        public static Parser<TExpression> BinaryExpression<TExpression, TOperator>(Parser<TExpression> terminalParser, Parser<TOperator> operatorParser, IEnumerable<Operation<TExpression, TOperator>> operations)
            => terminalParser.ThenAdd(Many(operatorParser.ThenAdd(terminalParser)))
            .Transform((x, y) => UnfoldExpression(x, y, operations));

        private static Operation<TExpression, TOperator> SingleOperation<TExpression, TOperator>(Associativity associativity, TOperator op, Func<TExpression, TExpression, TExpression> transformation)
            => new Operation<TExpression, TOperator>(associativity, new Dictionary<TOperator, Func<TExpression, TExpression, TExpression>>
            {
                { op, transformation },
            });

        private static TExpression UnfoldExpression<TExpression, TOperator>(TExpression head, IEnumerable<(TOperator, TExpression)> tail, IEnumerable<Operation<TExpression>> operations)
        {
            List<object> list = new List<object>();
            list.Add(head);
            foreach ((TOperator op, TExpression e) in tail)
            {
                list.Add(op);
                list.Add(e);
            }
        ...
        private static void UnfoldExpressionLeft<TExpression, TOperator>(List<object> list, Operation<TExpression> operation)
        {
            if (operation is Operation<TExpression, TOperator> binop)
            {
                ...
                {
                    if (binop.Transformations.TryGetValue((TOperator)list[index], out Func<TExpression, TExpression, TExpression> transformation))
                    {
